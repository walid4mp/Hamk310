...
async function main() {
  ...
  if(!user) user=await prisma.user.create({data:{name:'مدير النظام',email,passwordHash:await bcrypt.hash('MizanPro@123',12)}});
  let org=await prisma.organization.findFirst({where:{name:'شركة ميزان التجريبية'}});
  if(!org) org=await prisma.organization.create({data:{name:'شركة ميزان التجريبية',currency:'DZD'}});
  let branch=await prisma.branch.findFirst({where:{orgId:org.id,code:'HQ'}});
  if(!branch) branch=await prisma.branch.create({data:{orgId:org.id,code:'HQ',name:'المقر الرئيسي'}});
  ...
  for(const [code,name,type] of accounts) await prisma.account.upsert({where:{orgId_code:{orgId:org.id,code}},update:{},create:{orgId:org.id,code,name,type,isSystem:true}});
  ...
  if(cashAcc && !(await prisma.cashBox.findFirst({where:{orgId:org.id,name:'الصندوق الرئيسي'}}))) await prisma.cashBox.create({data:{orgId:org.id,branchId:branch.id,accountId:cashAcc.id,name:'الصندوق الرئيسي'}});
  const year=new Date().getFullYear(), start=new Date(year,0,1), end=new Date(year,11,31,23,59,59);
  if(!(await prisma.fiscalPeriod.findFirst({where:{orgId:org.id,startDate:start,endDate:end}}))) await prisma.fiscalPeriod.create({data:{orgId:org.id,name:`السنة المالية ${year}`,startDate:start,endDate:end,status:PeriodStatus.OPEN}});
  const sub=await prisma.subscription.findFirst({where:{orgId:org.id,active:true}});
  if(!sub){const startsAt=new Date(),endsAt=new Date(startsAt);endsAt.setMonth(endsAt.getMonth()+1);await prisma.subscription.create({data:{orgId:org.id,plan:SubscriptionPlan.MONTHLY,startsAt,endsAt,active:true}});}
